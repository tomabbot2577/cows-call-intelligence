...
# Monitor and auto-restart Layer 2 processing until all 1424 records are complete
# Checks every 5 minutes and starts new batches as needed

LOG_FILE="/tmp/layer2_monitor.log"
BATCH_SIZE=50
MAX_PARALLEL=10
TARGET_TOTAL=1424

echo "$(date): Starting Layer 2 monitoring loop" >> $LOG_FILE

while true; do
    # Get current count of enhanced records
    ENHANCED_COUNT=$(PGPASSWORD=call_insights_pass psql -U call_insights_user -d call_insights -h localhost -t -c "SELECT COUNT(*) FROM insights WHERE sentiment_reasoning IS NOT NULL;" | xargs)
    ...
    if [ "$ENHANCED_COUNT" -ge "$TARGET_TOTAL" ]; then
        ...
        echo "✅ COMPLETED: All $TARGET_TOTAL records have been enhanced with Layer 2 sentiment analysis"
        break
    fi
    ...
    if [ "$RUNNING" -eq 0 ] && [ "$REMAINING" -gt 0 ]; then
        ...
        for ((i=1; i<=BATCHES_NEEDED; i++)); do
            ...
            source /var/www/call-recording-system/venv/bin/activate && \
                python /var/www/call-recording-system/layer2_sentiment_enhanced.py --limit $BATCH_SIZE > /tmp/layer2_${BATCH_ID}.log 2>&1 &
    ...
    echo "📊 Progress: $ENHANCED_COUNT/$TARGET_TOTAL ($PERCENT%) | Running: $RUNNING processes | Remaining: $REMAINING"
